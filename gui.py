...
class MainWindow():
    def __init__(self, root):
        ...
        self.menu_bar_frame.pack(side='top', padx='5',
                                 pady='5', fill="x", expand=False)
# String-Entry
        string_entry = Entry(self.menu_bar_frame, bd=2, width=40)
        string_entry.pack(side='left', padx='5', pady='5', expand=False)
# Add-Button
        self.greet_button = Button(
            master=self.menu_bar_frame, text="Add to Filter", command=self.action)
        self.greet_button.pack(side='left', padx='5', pady='5', expand=False)
# Exit-Button
        self.greet_button = Button(
            master=self.menu_bar_frame, text="EXIT", command=self.action)
        self.greet_button.pack(side='right', padx='5', pady='5', expand=False)

        # self.createWindowFrame(self.root) # test

# ------------------------ list ------------------------

        canvas = Canvas(self.root, bg="green")
